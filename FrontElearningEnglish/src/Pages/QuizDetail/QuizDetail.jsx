                ...
                items.forEach(item => {
                    ...
                    if (type === "Question") {
                        ...
                        if (Array.isArray(groupQuestions)) {
                            // Attach FULL group info to each question in the group
                            // Fields from backend QuizItemDto:
                            const groupInfo = {
                                groupId: item.GroupId || item.groupId,
                                groupName: item.Name || item.name,           // Tên ngắn: Part 1, Part 2
                                groupTitle: item.Title || item.title,        // Tiêu đề: Photographs, Short Conversations
                                groupDescription: item.Description || item.description, // Đoạn văn/bài đọc
                                groupImgUrl: item.ImgUrl || item.imgUrl,
                                groupImgType: item.ImgType || item.imgType,  // image/jpeg, image/png
                                groupVideoUrl: item.VideoUrl || item.videoUrl,
                                groupVideoType: item.VideoType || item.videoType, // video/mp4, audio/mpeg
                                groupVideoDuration: item.VideoDuration || item.videoDuration, // seconds
                                groupSumScore: item.SumScore || item.sumScore, // Tổng điểm của group
                            };
                            
                            groupQuestions.forEach(q => {
                                allQuestions.push({
                                    ...q,
                                    _groupInfo: groupInfo // Add full group info as metadata
                                });
                            });
                        }
                    }
                ...
                if (Array.isArray(groups) && groups.length > 0) {
                    groups.forEach((group) => {
                        ...
                        if (Array.isArray(groupQuestions) && groupQuestions.length > 0) {
                            // Attach FULL group info to legacy structure as well
                            const groupInfo = {
                                groupId: group.GroupId || group.groupId || group.QuizGroupId || group.quizGroupId,
                                ...
                                groupImgType: group.ImgType || group.imgType,
                                groupVideoUrl: group.VideoUrl || group.videoUrl,
                                groupVideoType: group.VideoType || group.videoType,
                                groupVideoDuration: group.VideoDuration || group.videoDuration,
