...
export default function TeacherCourseDetail() {
  ...
  const handleCreateLessonSuccess = (newLessonData) => {
    setShowCreateLessonModal(false);
    setShowLessonSuccessModal(true);
    
    if (newLessonData) {
      // Update lessons list immediately with new lesson data
      setLessons(prevLessons => [...prevLessons, newLessonData]);
      // Update course lesson count
      setCourse(prevCourse => ({
        ...prevCourse,
        totalLessons: (prevCourse.totalLessons || 0) + 1
      }));
    } else {
