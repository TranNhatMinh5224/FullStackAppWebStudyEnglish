...
import { FaPlus, FaTrash, FaArrowUp, FaArrowDown, FaImage, FaVideo, FaTimes, FaLayerGroup, FaQuestionCircle, FaEye, FaEdit } from "react-icons/fa";
        ...
        <Tabs activeKey={activeTab} onSelect={(k) => setActiveTab(k)} className="px-3 pt-2 border-bottom-0 custom-tabs">
            <Tab eventKey="question" title={<span><FaQuestionCircle className="me-2"/>Câu hỏi</span>}>
                <div className="p-3 border-top"><div className="mb-3">
                    ...
                    <Form>
                        {/* Row 1: Loại câu hỏi + Điểm số + Media */}
                        <Row className="mb-4 align-items-start">
                            <Col md={4}>
                                <Form.Group>
                                    <Form.Label className="fw-semibold">Loại câu hỏi</Form.Label>
                                    <Form.Select value={qFormData.type} onChange={handleQTypeChange} disabled={!!questionToUpdate} className="shadow-sm">
                            ...
                            <Col md={3}>
                                <Form.Group>
                                    <Form.Label className="fw-semibold">Điểm số</Form.Label>
                                    <InputGroup className="shadow-sm">
                                        <Form.Control type="number" value={qFormData.points} onChange={(e) => setQFormData({ ...qFormData, points: e.target.value })}/>
                                        {internalGroupId && groupInfo && <InputGroup.Text className="bg-light text-muted small">/ {groupInfo.sumScore}</InputGroup.Text>}
                                    </InputGroup>
                                </Form.Group>
                            </Col>
                            <Col md={5}>
                                <Form.Group>
                                    <Form.Label className="fw-semibold">Media đính kèm (tùy chọn)</Form.Label>
                                    <div className="border rounded bg-light text-center media-upload-box">
                                        {!qMediaPreview ? (
                                            <div className="py-3 cursor-pointer" onClick={() => qFileInputRef.current?.click()}>
                                                <FaPlus size={18} className="text-muted mb-1 d-block mx-auto"/>
                                                <span className="text-muted small">{qUploadingMedia ? "Đang tải..." : "Ảnh/Video/Audio"}</span>
                                            </div>
                                        ) : (
                                            <div className="position-relative p-2">
                                                {qMediaType === 'image' && <img src={qMediaPreview} alt="Preview" className="img-fluid rounded" style={{maxHeight:'80px'}}/>}
                                                {qMediaType === 'video' && <video src={qMediaPreview} controls style={{maxHeight:'80px', width:'100%'}}/>}
                        ...
                        {/* Row 2: Nội dung câu hỏi - Markdown Editor Side-by-Side */}
                        <Form.Group className="mb-4">
                            <Form.Label className="fw-semibold">Nội dung câu hỏi (Markdown)</Form.Label>
                            <div className="markdown-editor-container">
                                <div className="markdown-editor-left">
                                    <div className="editor-label"><FaEdit className="me-1"/>Soạn thảo</div>
                                    <textarea
                                        className="markdown-textarea"
                                        value={qFormData.stemText}
                                        onChange={(e) => setQFormData({ ...qFormData, stemText: e.target.value })}
                                        placeholder={qFormData.type === QUESTION_TYPES.FillBlank 
                                ...
                                <div className="markdown-editor-right">
                                    <div className="editor-label"><FaEye className="me-1"/>Xem trước</div>
                                    <div className="markdown-preview">
                                        {qFormData.stemText.trim() ? (
                                            <ReactMarkdown remarkPlugins={[remarkGfm]}>{qFormData.stemText}</ReactMarkdown>
                                        ) : (
                                            <div className="markdown-preview-empty">Xem trước nội dung câu hỏi...</div>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="bg-light p-3 rounded border">
                                ...
                                {qFormData.type === QUESTION_TYPES.FillBlank && <div className="alert alert-info small mb-0">Dùng <code>[từ đúng]</code> trong nội dung câu hỏi để tạo chỗ trống.</div>}
                            </div>
                        </div>

                        {/* Row 4: Giải thích - Markdown Editor Side-by-Side */}
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-semibold">Giải thích đáp án (tùy chọn - Markdown)</Form.Label>
                            <div className="markdown-editor-container markdown-editor-small">
                                <div className="markdown-editor-left">
                                    <div className="editor-label"><FaEdit className="me-1"/>Soạn thảo</div>
                                    <textarea
                                        className="markdown-textarea"
                                        ...
                                        placeholder="Giải thích vì sao đáp án đó đúng/sai..."
                                    />
                                </div>
                                <div className="markdown-editor-right">
                                    <div className="editor-label"><FaEye className="me-1"/>Xem trước</div>
                                    <div className="markdown-preview">
                                        {qFormData.explanation?.trim() ? (
                                            <ReactMarkdown remarkPlugins={[remarkGfm]}>{qFormData.explanation}</ReactMarkdown>
                                        ) : (
                                            <div className="markdown-preview-empty">Xem trước giải thích...</div>
                                        )}
                                    </div>
                                </div>
                            </div>
