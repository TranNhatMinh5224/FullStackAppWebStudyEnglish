...
export default function CreateQuizGroupModal({ show, onClose, onSuccess, quizSectionId, groupToUpdate = null, isAdmin: propIsAdmin = false }) {
  ...
  useEffect(() => {
    if (show) {
      console.log("🔍 CreateQuizGroupModal - Roles:", roles);
      console.log("🔍 CreateQuizGroupModal - propIsAdmin:", propIsAdmin);
      console.log("🔍 CreateQuizGroupModal - isAdmin (detected):", isAdmin);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("📡 CreateQuizGroupModal - isAdmin:", isAdmin);
      console.log("📡 CreateQuizGroupModal - isUpdateMode:", isUpdateMode);
      console.log("📡 CreateQuizGroupModal - Will call:", isAdmin ? "ADMIN API" : "TEACHER API");
      ...
      if (isUpdateMode && groupToUpdate) {
        ...
        console.log("📡 Calling:", isAdmin ? "updateAdminQuizGroup" : "updateQuizGroup");
        response = isAdmin
          ? await quizService.updateAdminQuizGroup(groupId, updateData)
          : await quizService.updateQuizGroup(groupId, updateData);
      } else {
        console.log("📡 Calling:", isAdmin ? "createAdminQuizGroup" : "createQuizGroup");
        response = isAdmin
