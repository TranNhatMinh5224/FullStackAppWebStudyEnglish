...
public class StreakServiceTests
{
    ...
    [Fact]
    public async Task UpdateStreakAsync_WithConsecutiveDay_IncrementsStreak()
    {
        // Arrange
        var userId = 1;
        ...
        Assert.True(result.Success);
        Assert.NotNull(result.Data);
        Assert.Equal(6, result.Data.NewCurrentStreak);

        _streakRepositoryMock.Verify(x => x.UpdateAsync(It.IsAny<Streak>()), Times.Once);
    }

    [Fact]
    public async Task UpdateStreakAsync_WithSameDay_DoesNotUpdate()
    {
        // Arrange
        var userId = 1;
        ...
        Assert.True(result.Success);
        Assert.NotNull(result.Data);
        Assert.Contains("đã được cập nhật hôm nay", result.Data.Message);

        _streakRepositoryMock.Verify(x => x.UpdateAsync(It.IsAny<Streak>()), Times.Never);
    }

    [Fact]
    public async Task UpdateStreakAsync_WithBrokenStreak_ResetsTo1()
    {
        // Arrange
        var userId = 1;
